...
export function createAnalysisService({ getClient, clientSlug = APP_CLIENT.slug }) {
  ...
  async function saveCompleted(uploadId, result) {
    const response = await invoke(
      getClient(),
      "save-model-analysis",
      { uploadId, clientSlug, status: "completed", result },
  ...
  async function saveFailed(uploadId, errorCode) {
    const response = await invoke(
      getClient(),
      "save-model-analysis",
      { uploadId, clientSlug, status: "failed", errorCode },
      ANALYSIS_ERROR_CODES.ANALYSIS_SAVE_FAILED
    );
