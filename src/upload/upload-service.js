import { APP_CLIENT } from "../config/app.config.js";
...
export function createUploadService({
  ...
  async function uploadModel(file, metadata, { onCreated } = {}) {
    ...
    const createdUpload = await createModelUpload(
      client,
      { ...metadata, clientSlug },
      config
    );
    ...
    try {
      ...
      await bestEffortCleanup(client, createdUpload.uploadId, clientSlug);
  ...
  async function completeUpload(uploadId) {
    ...
    return completeUploadWithClient(client, uploadId, { clientSlug });
  }
...
async function completeUploadWithClient(client, uploadId, context = {}) {
  try {
    const completedUpload = await invokeFunction(
      ...
      { uploadId, clientSlug: context.clientSlug ?? APP_CLIENT.slug },
...
async function bestEffortCleanup(client, uploadId, clientSlug) {
  try {
    await invokeFunction(
      client,
      "remove-model-upload",
