...
import { resolveClientId } from "../_shared/client-resolver.ts";
...
Deno.serve(async (request: Request) => {
  ...
  const clientResolution = await resolveClientId(
    supabase,
    (payload as Record<string, unknown>)?.clientSlug
  );

  if (!clientResolution.valid) {
    return errorResponse(request, clientResolution.error.code, 400);
  }
  ...
  const { data: upload, error: uploadError } = await supabase
    ...
    .eq("client_id", clientResolution.clientId)
    .single();
